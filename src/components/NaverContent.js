...
function NaverContent({ naverNewsList }) {
  const [isNaverClicked, setIsNaverClicked] = useState(false);
  useEffect(() => {
    console.log('마운트 되었습니다.');
    setIsNaverClicked(true);
    return () => {
      console.log('언마운트 되었습니다.');
      setIsNaverClicked(false);
    };
  }, []);

  return (
    <div className={styles.content}>
      <Navigator isNaverClicked={isNaverClicked} />
      <div className={styles.content__items}>
        {naverNewsList &&
          naverNewsList.map((news, index) => {
            return (
